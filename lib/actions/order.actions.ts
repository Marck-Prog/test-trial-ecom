...
export async function updateOrderToPaid(orderId: string) {
  try {
    ...
    if (order.isPaid) throw new Error('Order already been paid')
    order.isPaid = true
...
export async function getMyOrders({
  ...
  const selectedCurrencyCode =
    cookieStore.get('currency')?.value || defaultCurrency

  // Find the currency details
  const selectedCurrency =
    availableCurrencies.find((c) => c.code === selectedCurrencyCode) ||
    availableCurrencies[0]

  // Convert the prices in the orders to the selected currency
  const convertedOrders = orders.map((order) => {
    const convertedPrice = round2(
      order.totalPrice * selectedCurrency.convertRate
    )
    return {
      ...order.toObject(),
      totalPrice: convertedPrice, // Store the converted price
      currency: selectedCurrency, // Pass the currency details
    }
  })

  return {
    data: JSON.parse(JSON.stringify(convertedOrders)),
    totalPages: Math.ceil(ordersCount / limit),
    currency: selectedCurrency, // Pass the currency for server-side rendering
  }
}
